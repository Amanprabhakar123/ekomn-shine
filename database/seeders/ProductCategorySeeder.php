...
class ProductCategorySeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        //     "Parent Category 1" => [
        //         "Sub Category 1" => [
        //             "Child Category 1" => ["Keyword 1", "Keyword 2"],
        //             "Child Category 2" => ["Keyword 3", "Keyword 4"]
        //         ],
        //         "Sub Category 2" => [
        //             "Child Category 3" => ["Keyword 5", "Keyword 6"]
        //         ]
        //     ],
        //     "Parent Category 2" => [
        //         "Sub Category 3" => [
        //             "Child Category 4" => ["Keyword 7", "Keyword 8"]
        //         ]
        //     ]
        // ];
        // dd($categories);
        ...
        // foreach ($categories as $mainCategory => $subCategories) {
        //     $mainCategorySlug = Str::slug($mainCategory);
        ...
        //         foreach ($childCategories as $childCategoryName => $keywords) {
        //             $childCategorySlug = Str::slug($childCategoryName);

        //             // Create child category
        //             $childCategoryData = Category::create([
        ...
        //                 // Create keyword as a category (depth 3)
        //                 Category::create([
        //                     'name' => $keyword,
        //                     'slug' => $keywordSlug,
        //                     'is_active' => true,
       ...
       $categories =[
        ...
        if (($handle = fopen($csvFile, 'r')) !== false) {
            ...
            while (($data = fgetcsv($handle)) !== false) {
                ...
                // Ensure there are at least three elements to define categories
                if (isset($data[0], $data[1], $data[2])) {
                    // Define the categories
                    $parentCategory = $data[0];
                    $subCategory = $data[1];
                    $childCategory = $data[2];
                    $keywords = array_slice($data, 3);

                    // Build the nested array
                    if (!isset($categories[$parentCategory])) {
                        $categories[$parentCategory] = [];
                    }
                    if (!isset($categories[$parentCategory][$subCategory])) {
                        $categories[$parentCategory][$subCategory] = [];
                    }
                    $categories[$parentCategory][$subCategory][$childCategory] = $keywords;
                }
            }
